...
class LowPassFilter(object):
    def __init__(self, fs, cf=50, order=2):
        self.fs = fs
        self.cf = cf
        self.order = order
        
    def lpf(self, arr, fs, cf, order):
        cf = 2*cf/fs
        sos = signal.butter(order, cf, btype = 'low', output = 'sos')     
        return signal.sosfiltfilt(sos, arr, padlen=250, padtype='even') 
                
    def __call__(self, mseq):
        for i, row in enumerate(mseq):
            mseq[i,:] = self.lpf(row, self.fs, self.cf,self.order)
        return mseq  
...
class AddNoise(object):
    ...
    def __init__(self,sigma=0.07, p = 0.5):
        #suggested sigma=0.07
        self.addnoise_p = p
        self.sigma=sigma
...
class NotchFilter(object):
    def __init__(self, fs=250, Q = 2, p = 0.5):
